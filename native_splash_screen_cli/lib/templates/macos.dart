...
Future<bool> _generateSourceFile({
  ...
  try {
    ...
    _writeConfigSection(buffer, config, imageData, build);
...
/// Writes the image data section of the Swift file
void _writeImageDataSection(StringBuffer buffer, BGRAImage imageData) {
  final length = imageData.data.length;

  buffer.writeln(
    '    // MARK: - Raw image data in (ARGB) format (Little Endian)',
  );
  buffer.writeln('    let imagePixels: [UInt8] = [');

  // Write data in a formatted way (8 values per line)
